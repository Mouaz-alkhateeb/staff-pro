  ...
  <section class="contact_area section_gap">
    <div class="container">
      <div class="d-flex justify-content-between align-items-center mb-3">
        <h2 class="mb-0">{{ t('cities') }}</h2>
        <router-link :to="{ name: 'add_city' }" class="btn btn-success btn-sm">
          <i class="bi bi-plus-circle"></i> {{ t('city.add') }}
        </router-link>
      </div>

      <form @submit.prevent="searchCities" class="row g-3 align-items-end mb-3">
        <div class="col-md-4">
          <label class="form-label">{{ t('city.name') }}</label>
          <input type="text" class="form-control" v-model="searchFilter.name"  :placeholder="t('city.name_placeholder')">
        </div>
        <div class="col-md-3">
          <label class="form-label">{{ t('city.status') }}</label>
          <select class="form-select" v-model="searchFilter.status">
            <option :value="undefined">{{ t('city.all') }}</option>
            <option :value="1">{{ t('city.active') }}</option>
            <option :value="0">{{ t('city.inactive') }}</option>
          </select>
        </div>
        <div class="col-md-3">
          <label class="form-label">{{ t('city.number_in_page') }}</label>
         <select
            class="form-select"
        ...
        <div class="col-md-2 d-flex gap-2">
          <button type="button" class="btn btn-danger" @click="resetFilter">{{ t('city.form.reset') }}</button>
        </div>
      </form>
        ...
        <table class="table table-hover align-middle">
          <thead>
            <tr>
              ...
              <th>{{ t('city.name') }}</th>
              <th>{{ t('city.status') }}</th>
              <th class="text-end">{{t('city.actions')}}</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="(city, index) in citiesList" :key="city.id">
              <td>{{ index + 1 + (paginationVar.page - 1) * paginationVar.per_page }}</td>
              <td>{{ city.name }}</td>
              <td>
                <span :class="['badge', city.status === 1 ? 'bg-success' : 'bg-secondary']">
                  {{ city.status === 1 ? 'نشط' : 'غير نشط' }}
                </span>
              </td>
              <td class="text-end">
                <button class="btn btn-outline-warning btn-sm me-1" @click="onEdit(city.id)">{{ t('city.update') }}</button>
                <button class="btn btn-outline-danger btn-sm" @click="onDelete(city.id)">{{ t('city.delete') }}</button>
              </td>
            </tr>
            <tr v-if="citiesList.length === 0">
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      ...
      <div class="modal-body">
        هل أنت متأكد أنك تريد حذف المدينة "<strong>{{ cityToDelete?.name }}</strong>"؟
      </div>
...
import { deleteCity, getCitiesList } from '@/services/City/cityService'
...
const { t } = useI18n()
const pageTitle = computed(() => t('cities'))
...
const resetFilter = () => {
  // searchFilter.value = defaultCitySearchFilter
...
onMounted(() => {
  fetchCities()
  const modalElement = document.getElementById('deleteCityModal')
  if (modalElement) {
...
const onEdit = (id: number | undefined) => router.push({ path: `/city/${id}/edit` })
const onDelete = (id: number | undefined) => {
  if (!id) return
  const city = citiesList.value.find((c) => c.id === id)
  if (city) {
...
const confirmDelete = async () => {
  if (!cityToDelete.value || typeof cityToDelete.value.id !== 'number') return

  const id = cityToDelete.value.id

  try {
    const { success } = await deleteCity(id)

    if (success) {
      const index = citiesList.value.findIndex((city) => city.id === id)
      if (index !== -1) {
